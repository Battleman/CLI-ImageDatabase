...
static void handle_read_call(struct mg_connection *nc, struct http_message *hm)
{
    ...
    //correcteur: pas vraiment besoin de le calloc, mais ca va aussi.
    ...
    for(size_t i = 0; i < MAX_QUERY_PARAM && result[i] != NULL; ++i) { //dès le premier NULL, il n'y a plus d'arguments
        //correcteur: vérifier que les index pair,lesautres sont du data.
        //correcteur: il faudrait vérifier que result[i] != NULL. Et vérifier que result[i] n'est pas autre chose que res ou pict_id, renvoyer un message d'erreur si c'est le cas.
    ...
    if(res == -1 || pict_id == NULL) {
        //correcteur: le message d'erreur c'est plutot ERR_RESOLUTIONS.
        mg_error(nc, ERR_INVALID_ARGUMENT);
    } else {
        int err = 0;
        char* img_buffer = NULL;
        uint32_t img_size = 0;
        //correcteur: il vaut mieux éviter ce genre de syntaxe, partez du fait que votre code sera relu, rendez cette relecture aussi simple que possible.
...
static void handle_insert_call(struct mg_connection *nc, struct http_message *hm)
{
    ...
    size_t img_len = 0;
    //correcteur: il est possible que tout ne soit pas parsé en une fois, il faudrait faire comme dans l'exemple avec n1 et n2.
...
static void handle_delete_call(struct mg_connection *nc, struct http_message *hm)
{
    ...
    //correcteur: pas besoin d'allouer.
    char* tmp = calloc((MAX_PIC_ID + 1) * MAX_QUERY_PARAM, sizeof(char));
    split(result, tmp, hm -> query_string.p, delim, hm -> query_string.len);
