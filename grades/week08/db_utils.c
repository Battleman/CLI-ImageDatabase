...
int overwrite_header(FILE* file, struct pictdb_header* header)
{
    ...
    /* Correcteur : peux importe, on est censé connaître la structure du fichier */
...
{
    /* Correcteur : d'accord, ça prends moins de lignes mais mieux vaut écrire son
     * code de manière bien explicite ligne par ligne. C'est visuellement moins
     * difficile à comprendre que de lire deux instructions dans un même if.
     */
    ...
    while(read_struct < max_files) {
        /* Correcteur : attention, cette manière de faire est couteuse en appels système */
...
{
    /* Correcteur : imaginons que db_file->fpdb soit NULL pour X raison mais
     * pas db_file... db_file->metadata ne sera jamais libéré.
     * -0.5pt
     */
