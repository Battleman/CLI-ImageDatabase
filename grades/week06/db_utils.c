...
int do_open(const char* filename, const char* mode, struct pictdb_file* db_file)
{
    ...
    if(db_file -> fpdb == NULL) {
        return ERR_FILE_NOT_FOUND; // Correcteur : je dirais plutot ERR_IO (pour être plus général)
    } else {
        /* Correcteur : ce bout de code posera problème plus tard : le tableau de metadata 
         * n'aura pas toujours comme taille MAX_MAX_FILES mais toujours header.max_files.
         * malus : -0.5pt
         */
...
{
    // Correcteur : de toute façon, appeller fclose sur un fichier non ouvert
    // donne un comportement indéfini selon le manpage
