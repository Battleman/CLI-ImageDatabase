...
do_list_cmd (const char* filename)
{
    ...
    /* Correcteur : en cas d'erreur, dans do_open, le fichier n'est pas fermé (après avoir été ouvert).
     * Pareil ici dans do_list_cmd.
     * malus : 0.5pt
     */
    if(fail == 0) {
        do_list(&myfile);
...
do_create_cmd (const char* filename)
{
    ...
    int errcode = 0;
    /* Correcteur : meilleure solution de s'occuper de l'ouverture du fichier dans do_create 
     * plutot qu'ici */
    myfile.fpdb = fopen(filename, "wb"); //n'est pas remplacé par do_open, car la lecture du fichier qu'on crée ne nous intéresse pas
    if(myfile.fpdb == NULL) {
        /* Correcteur : là aussi, ERR_IO préférable */
