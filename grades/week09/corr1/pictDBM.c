...
do_list_cmd (int argc, char *argv[])
{
    ...
    if(argc > 0) {
        ...
        //correcteur: si une erreur a eu lieu pendant l'ouverture, ce n 'est pas forcément nécessaire de fermer
        do_close(&db_file);
    }
    return errcode;
}
...
{
    //correcteur: ne mettez pas les valeurs en dur, utilisez des constantes ou des macros (cohérence avec les autres méthodes)
...
int do_insert_cmd(int argc, char *argv[])
{
    ...
    //correcteur: il faut free buffer
    do_close(&db_file);
