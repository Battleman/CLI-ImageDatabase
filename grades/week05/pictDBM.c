...
do_create_cmd (const char* filename)
{
    ...
    puts("Create");
    /* Correcteur : superflu. On peux directement créer un pictdb_file 
     * et assigner des champs d'une struct dans une struct. 
     * De plus, tous les champs pertinents ne sont pas initialisés
     * correctement. -0.25 + -0.75 = -1pt */
	struct pictdb_header header = {"", 0, 0, max_files, {small_res, thumb_res, 0}, 0, 0};
    struct pictdb_file fichier;
    fichier.header = header;
    /* Correcteur : le fichier est déjà ouvert dans do_create ! -1pt */
