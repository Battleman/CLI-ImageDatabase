...
//correcteur: unnecessary open close open
    ...
    for(size_t i = 0; i < original_db_file->header.max_files; i++) {
        if(original_db_file->metadata[i].is_valid == NON_EMPTY) {
            char* img = NULL;
            uint32_t img_size = 0;
            //correcteur: != 0 is redundant; not-zero is true by definition
            ...
            //correcteur: this is completely pointless; this is already called from do_insert and offset isn't set to 0 because you changed it in do_insert
            //correcteur: if this was an attempt of deduplication of small images, not correct
