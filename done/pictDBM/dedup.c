...
int do_name_and_content_dedup(struct pictdb_file* db_file, uint32_t index)
{
    ...
    int doublon = 0;
    uint32_t from_meta_index = 0; uint32_t to_meta_index = 0;
        ...
        if((i != index) && (NON_EMPTY == db_file -> metadata[i].is_valid)) {
            ...
            if(0 == table_compare(db_file -> metadata[index].SHA, db_file -> metadata[i].SHA, SHA256_DIGEST_LENGTH)) {
				for(int res = 0; res < NB_RES; res++) {
	                ...
	                db_file -> metadata[from_meta_index].offset[res] = db_file -> metadata[to_meta_index].offset[res];
	                db_file -> metadata[from_meta_index].size[res] = db_file -> metadata[to_meta_index].size[res];
				}
